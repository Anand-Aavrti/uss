...
const SERVICE_API_URL = 'https://yantramatrix-51nvouet.uc.gateway.dev/api/v1/service';
...
export default function EnrollClient() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setTimeout(() => {
        // Sibling-subdomain apps need a real navigation, not a client-side route.
        if (/^https?:\/\//.test(service.appUrl)) {
          window.location.assign(service.appUrl);
          return;
        }
