...
const Header = () => {
  ...
  // ✅ MDM Marketing Navigation — kept as-is
  const navigationItems = [
    { label: 'Overview', href: '/apps/mdm', icon: 'HomeIcon' },
    { label: 'Features', href: '/apps/mdm/features', icon: 'CubeIcon' },

    // 👇 Platforms ONLY for visitors
    !user && {
      label: 'Platforms',
      href: '/apps/mdm/platforms',
      icon: 'BuildingOfficeIcon',
    },
    !user && { label: 'Docs', href: '/apps/mdm/docs', icon: 'BookOpenIcon' },

    { label: 'Pricing', href: '/apps/mdm/pricing', icon: 'CurrencyDollarIcon' },
    { label: 'Contact', href: '/apps/mdm/contact', icon: 'PhoneIcon' },

    // 👇 Profile ONLY after login
    user && {
      label: 'Profile',
      href: '/apps/mdm/profile',
      icon: 'UserIcon',
    },
  ].filter((item): item is { label: string; href: string; icon: string } => Boolean(item)); // 🚨 IMPORTANT
  ...
  return (
    <>
      <header className="fixed top-0 inset-x-0 z-50">
        <div
          className={`transition-all duration-500 ${isScrolled ? 'px-4 lg:px-8 pt-4' : 'pt-4 px-4 lg:px-8'}`}
        >
          <div className="max-w-7xl mx-auto">
            ...
            >
              ...
              <div className="relative flex items-center justify-between h-[72px] px-6 lg:px-8">
                {/* ─── Logo ─── */}
                <Link
                  href="/apps/mdm"
                ...
                >
                  <div className="relative">
                    ...
                    >
                      <rect width="40" height="40" rx="8" fill="url(#mdm-logo-gradient)" />
                      <defs>
                        <linearGradient id="mdm-logo-gradient" x1="0" y1="0" x2="40" y2="40">
                      ...
                      {/* M */}
                      <path
                        d="M8 28V12L16 20L20 14L24 20L32 12V28"
                        stroke="#0EA5E9"
                        strokeWidth="2.5"
                        strokeLinecap="round"
                ...
                >
                  {navigationItems.map((item: { label: string; href: string; icon: string }) => {
                ...
                >
                  {!loading && !user && (
                    <>
                      {/* Login Button */}
                      <Link
                        href="/auth?mode=login&redirect=/apps/mdm/console"
                      ...
                      {/* Sign Up Button */}
                      <Link
                        href="/auth?mode=signup&redirect=/apps/mdm/console"
                  ...
                  {!loading && user && (
                    <button
                      ...
                      className="relative group px-6 py-2.5 text-sm font-medium text-red-400 rounded-full border border-red-500/30 hover:bg-red-500/10 transition-all"
                    >
      ...
      <ConfirmDialog
        ...
        title="Sign out from MDM?"
        description="You will be logged out from your account and redirected to the MDM overview page."
